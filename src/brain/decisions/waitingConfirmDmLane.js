  ...
  const avr =
    ...
    typeof ctx.activeAvailabilityRequest === "object"
      ...
      : facts.availabilityRequest && typeof facts.availabilityRequest === "object"
        ...
        : {};
  const expiresAtMs = avr.confirmExpiresAt
    ? new Date(avr.confirmExpiresAt).getTime()
    : null;
  return (
    facts.waitingConfirmTransactionActive === true &&
    clean(avr.status) === "approved" &&
    clean(avr.approvalCustomerNotificationStatus) === "sent" &&
    clean(avr.customerConfirmationStatus) === "waiting_confirm" &&
    !clean(avr.linkedBookingId) &&
    !clean(avr.supersededByAvailabilityRequestId) &&
