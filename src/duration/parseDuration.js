...
export function parseUserDuration(message) {
  ...
  if (withUnit) {
    ...
    const result = {
      ...
      unit,
      normalizedDays,
      ...(canonical === "hours" ? { normalizedHours: value } : {}),
    };
  ...
  if (/\b(half[\s-]?day|aadha\s+din|adha\s+din|half\s+din)\b/i.test(trimmed)) {
    const result = {
      value: 12,
      unit: "hours",
      normalizedDays: 1,
    ...
    console.log("[duration_parsed]", {
      ...
      unit: result.unit,
      normalizedDays: result.normalizedDays,
      normalizedHours: result.normalizedHours,
    });
    return result;
  }
...
export function getNormalizedDaysFromDurationPreference(pref) {
  ...
  if (
    ...
    if (/^(hour|hours|hr|hrs|ghanta|ghantay|ghanty|ghante|ghantey|ghnty|ghntay|ghnte|gnty|gntay|gnte|gantay|gante|gantey)$/.test(u)) {
      return Math.max(1, Math.ceil(pref.value / 24));
    }
